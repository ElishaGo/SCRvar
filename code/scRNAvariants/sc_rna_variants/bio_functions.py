...
class ReadsFilter(object):
    ...
    def process_read(self, read):
        ...
        #quality assurance
        low_quality_bases = sum(1 for read_quality in read.query_alignment_qualities if read_quality <= self.qual_threshold)
...
def create_filtered_bam(arguments):
    ...
    bamfile = open_bam(arguments.input_bam, arguments.threads)
    for i in range(20):
    ...
    temp_header_file = pathlib.Path(arguments.output_folder) / (os.path.basename(arguments.input_bam) +  '_temp_header.sam')
    ...
    sema = multiprocessing.Semaphore(arguments.threads)  # a shared variable, indicating number of concurrent processes
    all_processes = []
    basic_fileter = ReadsFilter(
            arguments.filtered_barcodes_list, arguments.min_mapq,
            arguments.cigar_clipping_allowed,
            arguments.max_gene_length, arguments.max_no_basecall,
            arguments.tag_for_umi, arguments.tag_for_cell_barcode)
    counter = 0
    ...
    for chunk in chunks_set:
        ...
        filtered_chunk_bam_path = pathlib.Path(arguments.output_folder) / names.pop()
        temporary_bams_paths.append(str(filtered_chunk_bam_path))
        p = multiprocessing.Process(
            target=filter_chromosome_chunk,
            args=(arguments.input_bam, chunk, filtered_chunk_bam_path,
    ...
    filtered_bam_path = str(pathlib.Path(arguments.output_folder) / (os.path.basename(arguments.input_bam) + "_filtered.bam"))
    pysam.merge("-f", "-h", str(temp_header_file), filtered_bam_path, *temporary_bams_paths, '--threads', str(arguments.threads))
    ...
    for filename in os.listdir(arguments.output_folder):
        if re.search('^%s_.*\.bam$' % os.getpid(), filename):
            os.remove(os.path.join(arguments.output_folder, filename))
...
def process_chromosome_chunk(
    ...
    for cell in cells_dict:
        for (direction, position) in cells_dict[cell]:
            ...
            total_singles_counts, total_multiples_counts, params = calculate_position_parameters(ref_base, cells_dict[cell][(direction, position)])
...
def variants_finder(filtered_bam, arguments):
    """ Given the filtered bam and script's argument, creates the final csv"""
    
    names_translator = ChromosomeNamesTranslator(arguments.genome_fasta)
    
    # devide the genome to chromosome chunks with limited amount of reads
    bamfile = open_bam(filtered_bam, arguments.threads)
    ...
    # create multiprocessing pool and helper variables
    pool = multiprocessing.Pool(arguments.threads,maxtasksperchild=1)
    asyncs = []
    ...
    # send threads to work
    for chunk in chunks_set:
        filtered_chunk_tsv_path = pathlib.Path(arguments.output_folder) / names.pop()
