...
(
    ...
    bedtools intersect -u -header -a ${EDITING_GTF_INTERSECT} -b ${OUTPUT_DIR}/2_${FNAME}.gene_filter_header.bam > ${OUTPUT_DIR}/2_${FNAME}_editing.bam_intersect.bed6

    # make intersection between SNP A_I sites transcriptomes and filtered bam file
    bedtools intersect -u -header -a ${SNP_GTF_INTERSECT} -b ${OUTPUT_DIR}/2_${FNAME}.gene_filter_header.bam > ${OUTPUT_DIR}/2_${FNAME}_snp.bam_intersect.vcf
