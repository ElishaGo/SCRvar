...
def order_and_save_agg(df, out_folder):
    ...
    save_df(df, out_folder, "aggregated_tsv.tsv")
...
def aggregate_df(df):
    ...
    per_position_stats = per_position_statistics(df)

    # merge the UMI statistics with table
    df_agg = df_agg.merge(per_position_stats, left_on='position', right_index=True)
    return df_agg
...
def load_tables(path, mutated=True):
    ...
    df.rename(columns={'chromosome': 'chrom', 'start': 'chromStart', 'end': 'chromEnd', 'same multi': 'same multi reads',
...
def run(args):
    ...
    df_mutated = load_tables(os.path.join(args.input_dir, "3_mismatch_dictionary.bed6"), mutated=True)
    df_unmutated = load_tables(os.path.join(args.input_dir, "3_no_mismatch_dictionary.bed6"), mutated=False)
    ...
    df_merged_agg = aggregate_df(df_merged)
    ...
    # save other tables
    save_df(df_merged, args.output_folder, "merged_mutated_unmutated_no_agg.tsv")
